...
def fixup(df):
    ...
    for index, row in df.iterrows():
        ...
        if (len(To) < 1 ) :
            df.loc[index, 'To'] = Family
    fn = 'Key to Families and Genera of the Order Dorylaimida Andrassy (2009).csv'
    df.to_csv(fn, encoding='utf-8',index=False)
    return df

def readSpreadSheet():
    fn = 'Key to Families and Genera of the Order Dorylaimida Andrassy (2009).xlsx'
