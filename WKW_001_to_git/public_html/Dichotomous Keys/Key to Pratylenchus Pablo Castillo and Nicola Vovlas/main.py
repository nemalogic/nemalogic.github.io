...
def fixup(df):
    ...
    for index, row in df.iterrows():
        ...
        To = str(row['To']).strip()
        Orders = str(row['Orders']).strip()
        if (len(To) < 1 ) :
            df.loc[index, 'To'] = Orders
    fn = 'Key to Orders of Nematodes.csv'
    df.to_csv(fn, encoding='utf-8',index=False)
    return df

def readSpreadSheet():
    fn = 'Key to Pratylenchus Pablo Castillo and Nicola Vovlas.xlsx'
...
def readCSV():
    fn = 'Key to Orders of Nematodes.xlsx'
    df = pd.read_csv(fn)
    df = df.fillna('')
    #df = fixup(df)
...
def draw(df):
    GraphTitle = 'Key to Pratylenchus\n Pablo Castillo and\n Nicola Vovlas'
    ...
    for index, row in df.iterrows():
        ...
        Description = str(row['Key to Pratylenchus Pablo Castillo and Nicola Vovlas']).strip()
